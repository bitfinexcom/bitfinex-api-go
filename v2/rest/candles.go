...
func getPathSegments(symbol string, resolution bitfinex.CandleResolution) (s string, err error) {
	if len(symbol) == 0 {
...
func (c *CandleService) Last(symbol string, resolution bitfinex.CandleResolution) (*candle.Candle, error) {
...
// See https://docs.bitfinex.com/reference#rest-public-candles for more info
func (c *CandleService) History(symbol string, resolution bitfinex.CandleResolution) (*candle.Snapshot, error) {
	segments, err := getPathSegments(symbol, resolution)
	if err != nil {
		return nil, err
	}
